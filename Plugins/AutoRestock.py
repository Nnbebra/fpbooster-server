...
async def update_status(pool, uid, msg):
    try:
        async with pool.acquire() as conn:
            await conn.execute("UPDATE autorestock_tasks SET status_message=$1, last_check_at=NOW() WHERE user_uid=$2::uuid", str(msg)[:100], uid)
    except: pass
...
async def save_settings(data: RestockSettings, req: Request, u=Depends(get_current_user_raw)):
    ...
    try:
        ...
        if not pool: raise Exception("Database pool not ready")

        await ensure_table_exists(pool)

        uid_str = str(u['uid']) # Принудительно в строку
        ...
        async with pool.acquire() as conn:
            ...
            try:
                ...
                current = await conn.fetchrow("SELECT lots_config FROM autorestock_tasks WHERE user_uid=$1::uuid", uid_str)
                ...
                logging.error(f"Error reading old config: {e}")
            ...
            """, uid_str, enc, data.active, json_str)
            
        return {"success": True, "message": "Настройки успешно сохранены"}

    except Exception as e:
        # Логируем в файл
        error_text = f"CRITICAL SAVE ERROR: {str(e)}\n{traceback.format_exc()}"
        print(error_text)
        logging.error(error_text)
        
        # Возвращаем 200 с описанием ошибки, чтобы клиент показал её пользователю
        return JSONResponse(status_code=200, content={
            "success": False, 
            "message": f"Server Err: {str(e)}"
        })
...
async def get_status(req: Request, u=Depends(get_current_user_raw)):
    try:
        ...
        return {"active": False, "message": f"Error: {str(e)}", "lots": []}
