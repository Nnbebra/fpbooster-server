...
async def admin_revoke_group(
    ...
    record_id: int = Form(...),  # Теперь принимаем ID конкретной записи
    _=Depends(admin_guard_ui)
):
    async with app.state.pool.acquire() as conn:
        # Удаляем строго одну конкретную строку
        await conn.execute("DELETE FROM user_groups WHERE id = $1", record_id)
